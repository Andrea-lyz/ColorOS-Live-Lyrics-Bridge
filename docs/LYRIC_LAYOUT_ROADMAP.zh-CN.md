...
> 当前实施状态：槽高按每行的换行、翻译和排版配置稳定计算，最低 `56dp`、预留约 `12dp` 垂直绘制空间和 `1dp` 底部安全空间，宽度尚不可用时才回退 `80dp`。字体外边界按 `top/bottom` 向外取整；所有主歌词最多同时显示两行，三行及以上保留完整换行集合并通过两行窗口展示。高度不依赖逐字进度、焦点、缩放、光晕或 AOD 状态；翻译开关使用统一布局进度同步收放可见行高，并补偿当前行锚点，AOD 只应用一次终态。行间距可在 `-5–20dp` 间以 `0.5dp` 调整，所有预设默认 `0dp`；当前行中心向下偏移 `48dp`。下文保留早期固定槽方案的设计过程与回归依据。
