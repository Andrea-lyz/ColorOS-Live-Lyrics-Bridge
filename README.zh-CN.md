...
> 当前版本：**v3.3.1**。升级时请把 Bridge 和已经安装的 LyricProvider 一起更新，避免新旧版本配合异常。
...
| 汽水音乐 | `LyricProvider-QiShui` | 逐字歌词、翻译歌词；还需完成下方特殊设置 |
...
3. 如果你使用 QQ 音乐、网易云、Apple Music、Poweramp、Spotify、汽水音乐或酷狗，再安装同一版本中对应的 `LyricProvider-*.apk`。
...
在 LSP 管理器中为汽水音乐开启“还原内联钩子”，并按管理器提示对列表中的应用处理 `libart.so`。跳过这一步时，汽水音乐可能提示版本不安全，Provider 也可能无法稳定工作。
