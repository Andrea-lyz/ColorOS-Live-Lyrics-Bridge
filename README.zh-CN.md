...
Release 附带面向 QQ 音乐、网易云音乐、Apple Music、Poweramp、Spotify、汽水音乐的可选 LyricProvider APK。它们是独立 LSPosed 模块，可同时为 ColorOS Live Lyrics Bridge 和 Lyricon/词幕转发完整歌词数据。
...
发布产物会命名为 `ColorOS-Live-Lyrics-Bridge-<tag>.apk`、`LyricProvider-QQMusic-<tag>.apk`、`LyricProvider-163Music-<tag>.apk`、`LyricProvider-AppleMusic-<tag>.apk`、`LyricProvider-Poweramp-<tag>.apk`、`LyricProvider-Spotify-<tag>.apk` 与 `LyricProvider-QiShui-<tag>.apk`。
