...
- Release 附带 `LyricProvider-Poweramp` APK，可为词幕和 ColorOS Live Lyrics Bridge 提供 Poweramp 本地/在线歌词。
- Release 附带 `LyricProvider-Spotify` APK，可为词幕和 ColorOS Live Lyrics Bridge 提供 Spotify 标准歌词。
- Poweramp、Spotify、QQ 音乐、网易云音乐 provider 均为独立辅助模块，需要按目标播放器分别安装并在 LSPosed 中启用。
